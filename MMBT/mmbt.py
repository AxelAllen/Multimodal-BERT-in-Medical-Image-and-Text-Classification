...
class ModalEmbeddings(nn.Module):
    ...
    def __init__(self, config, modal_hidden_size, encoder, embeddings):
        ...
        :param modal_hidden_size: Non-text modal embedding size (in this case the DenseNet hidden size)
        :param encoder: Non-text modal encoder (the pre-trained DenseNet)
        :param embeddings: Transformer embeddings
        """
        super().__init__()
        self.config = config
...
class MMBTModel(nn.Module, ModuleUtilsMixin):
    def __init__(self, config, mmbt_config):
        ...
        self.modal_encoder = ModalEmbeddings(config,
...
class MMBTForClassification(BertPreTrainedModel):
    ...
    **hidden_states**: (`optional`, returned when ``output_hidden_states=True``) list of ``torch.FloatTensor`` (one for
    ...
    def __init__(self, config, mmbt_config):
        ...
        self.mmbt = MMBTModel(config, mmbt_config)
